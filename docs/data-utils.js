 ...
 *
 * Nothing in this file touches the DOM. It only reads the data shapes
 * produced by aso_intelligence_engine.py:
 *   - history: the parsed comprehensive_aso_history.json object, shaped as
 *       { "<date>": { generated_at_utc, projects: { "<projectId>": {...} } } }
 *   - csvRows: the parsed aso_dashboard.csv rows (array of objects), each
 *       tagged with a "project" column
 *   - trackingConfig: the parsed tracking_config.json object, shaped as
 *       { projects: [ { id, label, app_id, keywords, countries, ... } ] }
 *
 * Kept separate from app.js so the logic here can be unit tested directly
 * in Node (see /tests/test-data-utils.js) without spinning up a browser.
 */
(function (root, factory) {
  ...
  /** Returns { id, label, app_id }[] for every project present on a given
   * date's snapshot, in the order tracking_config.json lists them (falling
   * back to whatever order the snapshot has them in, if the config isn't
   * available). */
  function listProjects(history, dateStr, trackingConfig) {
    const snapshot = (history[dateStr] || {}).projects || {};
    const order = (trackingConfig && trackingConfig.projects || []).map((p) => p.id);
    const ids = Object.keys(snapshot);
    ids.sort((a, b) => order.indexOf(a) - order.indexOf(b));
    return ids.map((id) => ({
      id,
      label: (snapshot[id] && snapshot[id].label) || id,
      app_id: snapshot[id] && snapshot[id].app_id,
    }));
  }

  function projectSnapshot(history, dateStr, projectId) {
    return ((history[dateStr] || {}).projects || {})[projectId] || {};
  }

  /**
   * Walks today's snapshot against the previous day's, for one project, and
   * produces a flat, human-readable list of concrete changes: rank moves,
   * new chart entrants, metadata field updates. This is the data behind
   * the Overview tab's "Signal feed".
   */
  function buildSignalFeed(history, dates, dateStr, projectId) {
    const idx = dates.indexOf(dateStr);
    const today = projectSnapshot(history, dateStr, projectId);
    const prevDateStr = idx > 0 ? dates[idx - 1] : null;
    const prev = prevDateStr ? projectSnapshot(history, prevDateStr, projectId) : null;
    const signals = [];
  ...
  /**
   * Flattens one project's snapshot into a searchable index used by the
   * global search box. Each item carries small "tags" (not a joined
   * string) so the UI can render them as separate badges.
   */
  function buildSearchIndex(history, dateStr, projectId) {
    const today = projectSnapshot(history, dateStr, projectId);
    const idx = [];
  ...
  /**
   * Builds a rank-over-time series for one project/keyword/country triple
   * from the flat CSV rows (module=keyword_rank, metric_name=rank).
   */
  function computeKeywordTrend(csvRows, projectId, keyword, country) {
    return csvRows
      .filter((r) => r.project === projectId && r.module === "keyword_rank" && r.metric_name === "rank"
  ...
  // ── tracking_config.json <-> form-field text conversions ────────────

  function countriesToText(arr) {
    return (arr || []).join(", ");
  }
  function textToCountries(text) {
  ...
  function categoriesToLines(catObj) {
    return Object.entries(catObj || {}).map(([id, label]) => `${id}=${label}`).join("\n");
  }
  function linesToCategories(text) {
    const out = {};
